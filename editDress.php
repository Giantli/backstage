...
<?php
    ...
    $weddingDress=getWeddingDressById($id);
    $pendulumName=$weddingDress['pendulumName'];
    $styleName=$weddingDress['styleName'];


?>
<body>
<div class="middle">
    <div class="header"><h2>修改婚纱</h2></div>
    <div class="addDressContainer">
        <form class="form-horizontal layui-form" method="post">
            <div class="form-group has-feedback">
                ...
                <div class="col-lg-8"><input type="text" class="form-control layui-input" name="name" lay-verify="name" value="<?php echo $weddingDress['name'] ?>" autocomplete="off" placeholder="请输入婚纱名"  ></div>
            </div>
            <div class="form-group has-feedback">
                <label class="control-label col-lg-2">价格：</label>
                <div class="col-lg-8"><input type="text" name="price" class="form-control" lay-verify="price" autocomplete="off" value="<?php echo $weddingDress['price'] ?>" placeholder="请输入价格"  ></div>
            </div>
            <div class="form-group has-feedback">
                    ...
                    <select class="form-control" name="pendulumId">
                        <?php if(is_array($pendulumList)){
                            foreach ($pendulumList as $item){
                                ?>
                                <option <?php echo $item['name']==$pendulumName?'selected':'' ?>  value="<?= $item['id'] ?>"><?= $item['name'] ?></option>
                    ...
                    <select class="form-control" name="styleId">
                        <?php if(is_array($styleList)){
                            foreach ($styleList as $item){
                                ?>
                                <option <?php echo $item['name']==$styleName?'selected':'' ?>  value="<?= $item['id'] ?>"><?= $item['name'] ?></option>
            ...
            <div class="form-group">
                ...
                <div class="col-lg-10 "><label class="uploadHeader" for="uploadHeader">修改图片</label> <input type="file" style="display: none"  id="uploadHeader"/>
                    <div  class="col-lg-12  imageContainer">
<!--                        --><?php //foreach ($weddingDress['images'] as $item){ ?>
<!--                            <div style="display: inline-block;position:relative;width:70px; margin-right:2px;"><img-->
<!--                                    src="images/--><?php //echo $item ?><!--" width="70"  alt=""></div>-->
<!--                        --><?php //} ?>
                    </div>
                </div>
